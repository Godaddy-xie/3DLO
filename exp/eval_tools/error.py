...
def cal_error(label,result_dict,flag):
    ret_list = []
    kong = 0
    cuo = 0
    for key, value in label.items():
        ...
        for index in range(count): 
            ...
            if(result_list == []):
                ret_list.append(NoneMatch(value[index],key))
                continue
            ...
            for result in result_list:
                IOU = get_iou(result,value[index])
                distance = computedis(result,value[index])
                if(flag == 'IOU'):
                    if(IOU>min_IOU):
                        min_IOU = IOU
                        label_Match = generate_item(result,value[index],IOU,key)
                    else:
                        continue
                    ...
                    if(distance<max_dis):
                        max_dis = distance
                        label_Match = generate_item(result,value[index],distance,key)
                    else:
                        continue

            if(flag == 'IOU'):           
                wrong_match_threshold = 0
                distance_index = -2
                if(label_Match[distance_index]<wrong_match_threshold):
                    label_Match = NoneMatch(value[index],key)
            else:
                wrong_match_threshold = 10
                distance_index = -1
                try:
                    if(label_Match[distance_index]>wrong_match_threshold):
                        label_Match = NoneMatch(value[index],key)
...
if __name__ == "__main__":
    ...
    result_path="/mnt/nfs/zzwu/04_centerNet/xjy/master_thesis/CenterNet-master/CenterNet-master/exp/ddd/bev/data/"
    ...
    new_path   =  "/mnt/nfs/zzwu/04_centerNet/xjy/master_thesis/CenterNet-master/CenterNet-master/exp/ddd/bev/val_error.csv"
    ...
    print('reusl suc')
    r = cal_error(label_data,result_data,'IOU')
    toCsv(r,new_path)
