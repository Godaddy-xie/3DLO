...
const double MIN_OVERLAP[3][3] = {{0.7, 0.5, 0.5}, {0.5, 0.25, 0.25}, {0.7, 0.5, 0.5}};
...
Polygon toPolygon(const T& g) {
    ...
    // //debug
    // cout<<"------w,l------"<<g.w<<" "<<g.l<<endl;
    // cout<<"not bbox"<<endl;  
    // for (int i = 0; i < 4; ++i) {
    //     cout<<"( "<<corners(0,i)<<" "<<corners(1,i)<<" )"<<" ";
    // }
    // cout<<endl;
...
inline double box3DOverlap(tDetection d, tGroundtruth g, int32_t criterion = -1) {
    ...
    //debug
    // cout<<endl;
    cout<<"inter_ara  "<<inter_area<<endl;
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  if(compute_fp){
    ...
    // if(metric == 2){
    // cout<<"---ig_th---"<<endl;
    // for(int32_t test=0; test<det.size(); test++){
    //   if (ignored_threshold[test]){
    //       cout<<"1"<<' ';}
    //   else{
    //     cout<<"0"<<' ';
    //   }
  ...
  // if(compute_fp){
  //    cout<<"ignored_gt"<<endl;
  //    for(int32_t test=0; test<gt.size(); test++){
  //      cout<<ignored_gt[test]<<" ";
  //    }
  //    cout<<endl;
  //    }
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    for(int32_t j=0; j<det.size(); j++){
      ...
      if(!compute_fp){
      cout<<"gt "<<i<<"det "<<j<<" "<<overlap<<endl;
      }
      // if(j == 2){
      // cout<<"-------------------finde-----------------"<<endl;
      // cout<<overlap<<endl;}
      // if(metric == 2 && compute_fp && i ==1 && j==1){
      //   cout<<"det "<<j<<" gt "<<i<<"  "<<overlap<<endl;
      //   cout<<assigned_detection[j]<<endl;
      // }
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  //debug
  cout<<"---------ignored_gt-------------"<<endl;
  for(int32_t j=0; j<ignored_gt[0].size(); j++){
    cout<<ignored_gt[0][j]<<' ';
  }
  cout<<endl;
  cout<<"---------ignored_det-------------"<<endl;
  for(int32_t j=0; j<ignored_det[0].size(); j++){
    cout<<ignored_det[0][j]<<' ';
  }
  cout<<endl;
  cout<<"---------thresholds-------------"<<endl;
  for(int32_t j=0; j<thresholds.size(); j++){
    cout<<thresholds[j]<<' ';
  }
  cout<<endl;
  cout<<"---------v-------------"<<endl;
  ...
  // cout<<"-----------"<<current_class<<"---------"<<difficulty<<endl;
  // cout<<n_gt<<endl;
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    //debug
    for(int32_t t=0; t<thresholds.size(); t++){
      cout<<"tp: "<<pr[t].tp<<"fp: "<<pr[t].fp<<"fn: "<<pr[t].fn<<endl;
    }

  }
  
  // for (int32_t i=0; i<thresholds.size(); i++){
  //   cout<<int(pr[i].fp)<<"--fp--"<<endl;
  //   cout<<int(pr[i].tp)<<"--tp--"<<endl;
  //   cout<<int(pr[i].fn)<<"--fn--"<<endl;
  // }
  // cout<<endl;
