...
from models.losses import FocalLoss, L1Loss, BinRotLoss ,DisentangelLOSS
...
#    self.dept_reg = ConfidenceLoss()
    self.bbox_loss = DisentangelLOSS()
    self.opt = opt
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      denominator  = torch.rsqrt(output['rot'][:,0:1,:,:]**2+output['rot'][:,0:1,:,:]**2)
      ...
      #todo ex
      Dis_loc_loss += self.bbox_loss(output['dep'],output['reg'],batch,'loc')
      Dis_rot_loss += self.bbox_loss(output['rot'],output['reg'],batch,'rot')
      Dis_dim_loss += self.bbox_loss(output['dim'],output['reg'],batch,'dim')
    ...
    loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'Dis_loc_loss': Dis_loc_loss, 
                  'Dis_rot_loss': Dis_rot_loss, 'Dis_dim_loss': Dis_dim_loss,'wh_loss':wh_loss
                  }
...
class DddTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'Dis_loc_loss', 'Dis_rot_loss', 'Dis_dim_loss','wh_loss']
    #loss_states = ['loss', 'hm_loss', 'dep_loss', 'dim_loss', 'rot_loss', 
    #                'off_loss']
    #loss_states = ['loss', 'hm_loss', 'Dis_loc_loss','Dis_rot_loss']
    loss = DddLoss(opt)
