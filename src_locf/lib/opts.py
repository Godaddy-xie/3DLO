...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--loc_weight', type=float, default=1,
                             help='loss weight for depth and offset.')
    self.parser.add_argument('--dim_weight', type=float, default=1,
                             help='loss weight for 3d bounding box size.')
    self.parser.add_argument('--rot_weight', type=float, default=1,
                             help='loss weight for orientation.')
    self.parser.add_argument('--reg_weight', type=float, default=1,
                             help='loss weight for orientation.')
    self.parser.add_argument('--peak_thresh', type=float, default=0.25)
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'reg_3d': 8,'conf':1}
      # if opt.reg_bbox:
      #   opt.heads.update(
      #     {'wh': 2})
      # if opt.reg_offset:
