...
class Reg3dLoss_IOU(nn.Module):
    ...
    def forward(self, output, target):
        ...
        loss_dims = F.l1_loss(pred_bbox3d_dims, gt_bbox3d, reduction='sum') / (ind.shape[1] * 10)
        loss_locs = F.l1_loss(pred_bbox3d_locs, gt_bbox3d, reduction='sum') / (ind.shape[1] * 10)
        return  (loss_rotys+loss_dims+loss_locs),iouloss
...
class Conf_Loss(nn.Module):
    ...
    def forward(self, output,output_conf,target):
        batch_size = len(target['img_id'])
       ...
       # import pdb;pdb.set_trace()
        ...
        gt_bbox3d = points_decode(gt_rotys, gt_dimensions, gt_locations)
        #import pdb;pdb.set_trace()
        iouloss = torch.tensor(giou_3D_loss(iou_bbox,gt_bbox3d)).cuda().float()
        conf = _tranpose_and_gather_feat(output_conf, ind).view(-1,1)
        mask = mask.float().view(-1,1)
        
        conf = conf*mask
        loss_conf =  F.l1_loss(conf, iouloss, reduction='sum') / (ind.shape[1])
        return  loss_conf
