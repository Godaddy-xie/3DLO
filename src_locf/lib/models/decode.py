...
def _topk(scores, K=40):
    ...
    # topk_scores, topk_inds in shape [N, cat, max_det]
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)

    # get the x,y value of top K results
    topk_inds = topk_inds % (height * width)
    topk_ys = (topk_inds / width).int().float()
    topk_xs = (topk_inds % width).int().float()
...
def sincos_3d_decode(heat, reg,conf ,K=100):
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
    ...
    if reg is not None:
        reg = _tranpose_and_gather_feat(reg, inds)
        conf = _tranpose_and_gather_feat(conf,inds)
        conf =conf.squeeze(2)
        #import pdb;pdb.set_trace()
        scores = scores*(1-conf)
        reg = reg.view(batch, K, 8)
        ct3d_offset = reg[:, :, 1:3]
        delta_depth = reg[:, :, 0].unsqueeze(2)
        delta_dim = torch.sigmoid(reg[:, :, 3:6]) - 0.5
        delta_ori = F.normalize(reg[:, :, 6:8], dim=2)
        xs, ys = xs.unsqueeze(2), ys.unsqueeze(2)
        scores = scores.unsqueeze(2)
        clses = clses.unsqueeze(2).float()
        detections = torch.cat(
            [xs, ys, ct3d_offset, scores, delta_depth, delta_dim, delta_ori, clses], dim=2)

    return detections


def baidu_decode(heat, reg, K=100):
    batch = heat.size()[0]
    heat = _nms(heat)
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    ...
    if reg is not None:
        reg = _tranpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 9)
        ct3d_offset = reg[:, :, 1:3]
        delta_depth = reg[:, :, 0].unsqueeze(2)
        yaw = F.normalize(reg[:, :, 3:5], dim=2)
        alpha = F.normalize(reg[:, :, 5:7], dim=2)
        roll = F.normalize(reg[:, :, 7:9], dim=2)
        xs, ys = xs.unsqueeze(2), ys.unsqueeze(2)
        scores = scores.unsqueeze(2)
        clses = clses.unsqueeze(2).float()
        detections = torch.cat(
            [xs, ys, ct3d_offset, scores, delta_depth, yaw, alpha, roll, clses], dim=2)

    return detections


def baidu_multibin_decode(heat, depth, reg, alpha, yawroll, K=100):
    batch, cat, height, width = heat.size()
    heat = _nms(heat)
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    ...
    alpha = _tranpose_and_gather_feat(alpha, inds)
    alpha = alpha.view(batch, K, 6)
    # todo: do we need softmax here?
    alpha[:, :, 2:4] = F.normalize(alpha[:, :, 2:4], dim=2)
    alpha[:, :, 4:] = F.normalize(alpha[:, :, 4:], dim=2)
    yawroll = _tranpose_and_gather_feat(yawroll, inds)
    yawroll = yawroll.view(batch, K, 4)
    yawroll[:, :, :2] = F.normalize(yawroll[:, :, :2], dim=2)
    yawroll[:, :, 2:] = F.normalize(yawroll[:, :, 2:], dim=2)

    scores = scores.view(batch, K, 1)
    detections = torch.cat([xs, ys, depth, alpha, yawroll, scores], dim=2)
    return detections
