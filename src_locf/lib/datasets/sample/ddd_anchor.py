...
import json
machtdict = json.load(open('/mnt/nfs/zzwu/04_centerNet/xjy/master_thesis/CenterNet-master/CenterNet-master/src/match.json','r'))
...
import pycocotools.coco as coco
def anchor_Z(v,depth,matchdict):
   v = str(v)
   anchor_z = machtdict[v][0]
   resdiual = float(depth) - anchor_z
   return anchor_z,resdiual
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        if 1: 
          ...
          v = ct_int[1]
          anchor_z,resdiual = anchor_Z(v,ann['depth'],machtdict)
