...
def get_alpha_smoke(rot):
  tanv = rot[:,0] / rot[:,1]
  thet = np.arctan(tanv)+np.pi*0.5
  return thet
...
def ddd_post_process_2d_smoke(dets, c, s, opt):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  ret = []
  ...
  for i in range(dets.shape[0]):
    ...
    for j in range(opt.num_classes):
      ...
      top_preds[j + 1] = np.concatenate([
        dets[i, inds, :3].astype(np.float32),
        get_alpha_smoke(dets[i, inds, 3:5])[:, np.newaxis].astype(np.float32),
...
def ddd_post_process(dets, c, s, calibs, opt):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  dets = ddd_post_process_2d_smoke(dets, c, s, opt)
  dets = ddd_post_process_3d(dets, calibs)
  return dets
