...
from models.losses3d import Reg3dLoss,Reg3dLoss_IOU
...
class DddLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        hm_loss, reg_loss,iou_loss = 0., 0. ,0.
        for s in range(opt.num_stacks):
            ...
            iou_loss     += 0.1*self.crit_reg(output['reg_3d'], batch) /opt.num_stacks
            #reg_loss+=reg / opt.num_stacks
            # if opt.dep_weight > 0:
        ...
        loss = opt.hm_weight * hm_loss + opt.reg_weight * reg_loss+1*iou_loss
