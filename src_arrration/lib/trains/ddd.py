...
from models.losses3d import Reg3dLoss,Reg3dLoss_IOU,Conf_Loss
...
class DddLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        hm_loss, reg_loss,conf_loss = 0., 0. ,0.
        for s in range(opt.num_stacks):
            output = outputs[s]
            output['hm'] = _sigmoid(output['hm'])
            output['conf'] = _sigmoid(output['conf'])
            # sigmoid on dimensions
            output['reg_3d'][:, 3:6, :, :] = torch.sigmoid(output['reg_3d'][:, 3:6, :, :].clone()) - 0.5
            output['reg_3d'][:, 6:, :, :] = F.normalize(output['reg_3d'][:, 6:, :, :].clone())
            # follow the origin depth estimation in centernet
            # output['reg_3d'][:, 0, :, :] = 1. / (output['reg_3d'][:, 0, :, :].sigmoid() + 1e-6) - 1.
            ...
            reg_loss+= self.crit_reg(output['reg_3d'], batch) / opt.num_stacks
            conf_loss += self.conf(output['reg_3d'],output['conf'],batch)  / opt.num_stacks
        ...
        loss = opt.hm_weight * hm_loss + opt.reg_weight * reg_loss +3 * conf_loss
...
class DddTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'reg_loss','conf_loss']
        loss = DddLoss(opt)
