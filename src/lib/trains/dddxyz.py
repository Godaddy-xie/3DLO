...
from models.decode import ddd_decode,ddd_decode_xyz
from models.utils import _sigmoid
from utils.debugger import Debugger
from utils.post_process import ddd_post_process,ddd_post_process_xyz
...
class DddLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    ...
    hm_loss, location_loss, rot_loss, dim_loss = 0, 0, 0, 0
    wh_loss, off_loss = 0, 0
    for s in range(opt.num_stacks):
      output = outputs[s]
      output['hm'] = _sigmoid(output['hm'])
      output['location'][:,2,:,:] = 1. / (output['location'][:,2,:,:].sigmoid() + 1e-6) - 1.
      
      if opt.eval_oracle_dep:
        output['location'] = torch.from_numpy(gen_oracle_map(
          batch['location'].detach().cpu().numpy(), 
          batch['ind'].detach().cpu().numpy(), 
          opt.output_w, opt.output_h)).to(opt.device)
      
      hm_loss += self.crit(output['hm'], batch['hm']) / opt.num_stacks
      if opt.location_weight > 0:
        location_loss += self.crit_reg(output['location'], batch['reg_mask'],
                                  batch['ind'], batch['location']) / opt.num_stacks
      if opt.dim_weight > 0:
    ...
    loss = opt.hm_weight * hm_loss + opt.location_weight * location_loss + \
           opt.dim_weight * dim_loss + opt.rot_weight * rot_loss + \
           opt.wh_weight * wh_loss + opt.off_weight * off_loss

    loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'location_loss': location_loss, 
...
class DddTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'location_loss', 'dim_loss', 'rot_loss', 
                   'wh_loss', 'off_loss']
    loss = DddLoss(opt)
  ...
  def debug(self, batch, output, iter_id):
      ...
      # if opt.dataset == 'gta':
      #   dets[:, 12:15] /= 3
      dets_pred = ddd_post_process_xyz(
