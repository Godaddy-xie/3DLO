...
import numpy as np
from .image import transform_preds,transform_preds_kps
from .ddd_utils import ddd2locrot,ddd2locrot_xyz,ddd2locrot_rtm
...
def ddd_post_process_2d(dets, c, s, opt):
  ...
  include_wh = dets.shape[2] > 43

  for i in range(dets.shape[0]):
    top_preds = {}
    # print(dets[i,0,0:2])
    # print(dets[i,0,15:17])
    # print(dets[i,0,:])
    # assert 0
    dets[i, :, :2] = transform_preds(
          dets[i, :, 0:2], c[i], s[i], (opt.output_w, opt.output_h))
    dets[i,:,17:35] = transform_preds_kps(
          dets[i, :, 17:35], c[i], s[i], (opt.output_w, opt.output_h))
    dets[i,:,15:17] = transform_preds(
          dets[i, :,15:17], c[i], s[i], (opt.output_w, opt.output_h))
    classes = dets[i, :, -1]
    s = dets[i,:,2]
    for j in range(opt.num_classes):
      
      inds_main_score = (s>0.5)
      inds_cls = (classes == j)
      #TODO with kp_scores
      
      inds = inds_main_score & inds_cls
      ...
      top_preds[j+1] = np.concatenate([top_preds[j+1],dets[i,inds,17:44].astype(np.float32)],axis =1) 
  ...
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        locations, rotation_y = ddd2locrot_rtm(
          center, alpha, dimensions, depth,kp,scores,calibs[0])
  ...
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        ##----xiugai
        # locations, rotation_y = ddd2locrot_(
        #   center, alpha, dimensions, depth, calibs[0])
        locations ,rotation_y ,dimensions = ddd2locrot_rtm(
           center, alpha, dimensions, depth, calibs[0])
        bbox = [center[0] - wh[0] / 2, center[1] - wh[1] / 2,
                center[0] + wh[0] / 2, center[1] + wh[1] / 2]
        pred = [alpha] + bbox + dimensions.tolist() + \
...
def ddd_post_process_3d_rtm(dets, calibs):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  ret = []
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        dimensions = dets[i][cls_ind][j][5:8]
        wh = dets[i][cls_ind][j][8:10]
        kp = dets[i][cls_ind][j][10:28]
        score_kp =dets[i][cls_ind][j][28:37]
    
        ##----xiugai
        # locations, rotation_y = ddd2locrot_(
        #   center, alpha, dimensions, depth, calibs[0])
        locations ,rotation_y ,dimensions = ddd2locrot_rtm(
           center, alpha, dimensions, depth,kp,score_kp,calibs)
        bbox = [center[0] - wh[0] / 2, center[1] - wh[1] / 2,
                center[0] + wh[0] / 2, center[1] + wh[1] / 2]
        pred = [alpha] + bbox + dimensions.tolist() + \
               locations.tolist() + [rotation_y, score]
...
def ddd_post_process_2d_xyz(dets, c, s, opt):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  ret = []
  ...
  for i in range(dets.shape[0]):
    ...
    for j in range(opt.num_classes):
      ...
      top_preds[j + 1] = np.concatenate([
        dets[i, inds, :3].astype(np.float32),
        get_alpha(dets[i, inds, 3:11])[:, np.newaxis].astype(np.float32),
        get_pred_depth(dets[i, inds, 11:14]).astype(np.float32),
...
def ddd_post_process_3d_xyz(dets, calibs):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  ret = []
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        score = dets[i][cls_ind][j][2]
        alpha = dets[i][cls_ind][j][3]
        locations = dets[i][cls_ind][j][4:7]
        dimensions = dets[i][cls_ind][j][7:10]
        wh = dets[i][cls_ind][j][10:12]
        locations, rotation_y = ddd2locrot_xyz(
          center, alpha, dimensions, locations, calibs[0])
        bbox = [center[0] - wh[0] / 2, center[1] - wh[1] / 2,
                center[0] + wh[0] / 2, center[1] + wh[1] / 2]
        pred = [alpha] + bbox + dimensions.tolist() + \
               locations.tolist() + [rotation_y, score]
...
def ddd_rtm_post_process(dets, c, s, calibs, opt):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  dets = ddd_post_process_2d(dets, c, s, opt)
  dets = ddd_post_process_3d_rtm(dets, calibs)
  return dets
...
def ddd_post_process_xyz(dets, c, s, calibs, opt):
  # dets: batch x max_dets x dim
  # return 1-based class det list
  dets = ddd_post_process_2d_xyz(dets, c, s, opt)
  dets = ddd_post_process_3d_xyz(dets, calibs)
  return dets
