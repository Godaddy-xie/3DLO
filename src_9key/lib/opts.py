...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet |dddxyz')
    ...
    # ddd
    self.parser.add_argument('--dep_weight', type=float, default=0.1,
    ...
    self.parser.add_argument('--kp_weight', type=float, default=1,
                             help='loss weight for orientation.')
    self.parser.add_argument('--vertor_weight', type=float, default=1,
                             help='loss weight for orientation.')
    self.parser.add_argument('--peak_thresh', type=float, default=0.2)

    #dddxyz
    self.parser.add_argument('--location_weight', type=float, default=1,
                             help='loss weight for location_weight.')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'dep': 1, 'rot': 8, 'dim': 3,"hm_ver":9,"vertor":18}
      if opt.reg_bbox:
        opt.heads.update(
          {'wh': 2})
      if opt.reg_offset:
    ...
    elif opt.task == 'dddxyz':
      # assert opt.dataset in ['gta', 'kitti', 'viper']
      opt.heads = {'hm': opt.num_classes, 'location': 3, 'rot': 8, 'dim': 3}
      if opt.reg_bbox:
        opt.heads.update(
          {'wh': 2})
      if opt.reg_offset:
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'ddd': {'default_resolution': [384, 1280], 'num_classes': 3, 
                ...
                'dataset': 'kitti'},
      'dddxyz': {'default_resolution': [384, 1280], 'num_classes': 3, 
                'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225],
                'dataset': 'kitti'},
    }
    class Struct:
