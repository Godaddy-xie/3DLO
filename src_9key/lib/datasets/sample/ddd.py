...
class DddDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    ##rtm3d
    hm_vector = np.zeros((9,self.opt.output_h,self.opt.output_w),dtype = np.float32)
    vector = np.zeros((self.max_objs,18),dtype = np.float32)
    ...
    for k in range(num_objs):
      ...
      points  = self._get_keypoint(ann)    

      fx, cx, fy, cy = calib[0,0], calib[0,2], calib[1,1], calib[1,2]
      K = np.array([fx, 0, cx, 0, fy, cy, 0, 0, 1])
      K = K.reshape(3,3)
      points = points.T
      point_2d = np.dot(K, points)
      point_2d = point_2d / point_2d[2]
      point_2d = point_2d.T
      point_2D_center = np.ones((9,2))
      
      for i in range(9):
        point_2D_center[i]  = affine_transform(point_2d[i][0:2],trans_output)
      #------------------------------------------------------------------------#
      #-------------------------mainpoint ande verter--------------------------#
      #------------------------------------------------------------------------#
      ...
      if h > 0 and w > 0:
        ...
        ##----9 keypoint-------#
        for  pid in range(9):
          point_2d_hm =  point_2D_center[pid]
          ct = np.array(
          [point_2d_hm[0], point_2d_hm[1]], dtype=np.float32)
          if(ct[0]>self.opt.output_w-1):
            ct[0] = self.opt.output_w-1
          elif(ct[0]<0):
            ct[0] = 0  
          if(ct[1]>self.opt.output_h-1):
            ct[1] =self.opt.output_h-1
          elif(ct[1]<0):
            ct[1] =0  
          ct_vec.append(ct)
          ct_int = ct.astype(np.int32)
          ct_vec_int.append(ct_int)
          draw_gaussian(hm_vector[pid], ct, radius)
        #--------------#
        #for main point#
        #--------------#
      ...
      #------------------------------------------------------------------------#
        for num in range(9):
            vector[k][2*num:2*(num+1)] = ct_vec[num] -ct
        ...
        if 1: 
          ...
          #depth_weight[k] = 1./(1+np.exp(-dep[k]))
    ...
    ret = {'input': inp, 'hm': hm, 'dep': dep, 'dim': dim, 'ind': ind, 
           ...
           'rot_mask': rot_mask,'hm_ver':hm_vector,"vertor":vector}
    if self.opt.reg_bbox:
      ret.update({'wh': wh})
  ...
  def _alpha_to_8(self, alpha):
    ...
    return ret
  def _get_keypoint(self,ann):
      locations  = list(map(float,ann['location']))
      dim     = list(map(float,ann['dim']))
      h = dim[0]
      rot     = float(ann['rotation_y'])
      bbox = np.array(compute_box_3d(dim,locations,rot))
      center = np.array([[locations[0],locations[1]-0.5*h,locations[2]]])
      ret = np.vstack((bbox,center))
      return ret
