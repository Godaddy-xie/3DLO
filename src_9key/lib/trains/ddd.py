...
from models.losses import FocalLoss, L1Loss, BinRotLoss ,ConfidenceLoss,L2Loss,L1Loss_ver
...
class DddLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    ...
    hm_loss, dep_loss, rot_loss, dim_loss,kp_loss,vertor_loss = 0, 0, 0, 0, 0, 0
    wh_loss, off_loss = 0, 0
    for s in range(opt.num_stacks):
      output = outputs[s]
      output['hm'] = _sigmoid(output['hm'])
      output['hm_ver'] = _sigmoid(output['hm_ver'])
      output['dep']= 1. / (output['dep'].sigmoid() + 1e-6) - 1.
      # output['dep'][:,:,:,1] = output['dep'][:,:,:,1].sigmoid()
      #output['dep'] = torch.log(output['dep'])
      ...
      kp_loss += self.crit(output['hm_ver'], batch['hm_ver']) / opt.num_stacks
      
      vertor_loss +=self.vec_reg(output['vertor'],batch['reg_mask']
                              ,batch['ind'],batch['vertor']) / opt.num_stacks
      

      if opt.dep_weight > 0:
        dep_loss += self.depth_reg(output['dep'], batch['reg_mask'],
                                  batch['ind'], batch['dep']) / opt.num_stacks
      if opt.dim_weight > 0:
        dim_loss += self.dim_reg(output['dim'], batch['reg_mask'],
    ...
    loss = opt.hm_weight * hm_loss + opt.dep_weight * dep_loss + \
           ...
           1.1*opt.kp_weight * kp_loss + opt.vertor_weight *vertor_loss
    #loss = opt.hm_weight * hm_loss+ opt.vertor_weight *vertor_loss

    loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'dep_loss': dep_loss, 
                  'dim_loss': dim_loss, 'rot_loss': rot_loss, 
                  'wh_loss': wh_loss, 'off_loss': off_loss,'kp9_loss':kp_loss,'vertor_loss':vertor_loss}
...
class DddTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'dep_loss', 'dim_loss', 'rot_loss', 
                   'wh_loss', 'off_loss','kp9_loss','vertor_loss']
    #loss_states = ['loss', 'hm_loss', 'dep_loss', 'dim_loss', 'rot_loss', 
    #                'off_loss']
    loss = DddLoss(opt)
