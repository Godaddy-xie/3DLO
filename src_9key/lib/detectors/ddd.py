...
from models.decode import ddd_decode,ddd_decode_rtm
from models.utils import flip_tensor
from utils.image import get_affine_transform
from utils.post_process import ddd_post_process,ddd_rtm_post_process
...
class DddDetector(BaseDetector):
  ...
  def process(self, images, return_time=False):
    with torch.no_grad():
      ...
      output['hm_ver'] = output['hm_ver'].sigmoid_()
      output['dep'] = 1. / (output['dep'].sigmoid() + 1e-6) - 1.
      # print(output['hm'][:,:,65:71,168:174])
      # print("--------------------------------")
      #output['dep'] = torch.exp(output['dep'])
      ...
      dets = ddd_decode_rtm(self.opt,output['hm'], output['rot'], output['dep'],
                          output['dim'],output['hm_ver'],output['vertor'],wh=wh, reg=reg, K=self.opt.K)
    if return_time:
  ...
  def post_process(self, dets, meta, scale=1):
    ...
    detections = ddd_rtm_post_process(
