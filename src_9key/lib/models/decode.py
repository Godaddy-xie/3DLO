...
import json
import numpy as np
#matchdict = json.load(open('/mnt/nfs/zzwu/04_centerNet/xjy/master_thesis/CenterNet-master/CenterNet-master/src/match.json','r'))
...
def exct_decode(
    ...
    return detections
def ddd_decode_rtm(opt,heat, rot, depth, dim, kp,ver,wh=None, reg=None, K=40):
    ...
    ver = _transpose_and_gather_feat(ver, inds)
    
    kp_coord, kp_scores = get_kp(ver,inds,kp,opt)
    kp_coord = kp_coord.cuda().float()
    kp_scores =kp_scores.cuda().float()
    ...
    if wh is not None:
        ...
        detections = torch.cat(
            [xs, ys, scores, rot, depth, dim,wh,kp_coord,kp_scores, clses], dim=2)
    else:
        detections = torch.cat(
            [xs, ys, scores, rot, depth, dim,  kp_coord,kp_scores,clses], dim=2)
...
def ddd_decode(heat, rot, depth, dim, wh=None, reg=None, K=40):
    ...
    return detections
def find_closed2maiVc(kp,ind,ver,opt):
    
    w =opt.output_w
    h =opt.output_h
    _ys   = (ind / w).int().float()
    _xs   = (ind % w).int().float()
    MP = torch.tensor([_xs,_ys]).cuda()
    kp_ver = ver.reshape(-1,2)+MP
    kp_int = kp_ver.int()
    map_id = {0:[-1,-1],1:[0,1],2:[1,-1],
            3:[-1,0],4:[0,0],5:[1,0],
            6:[-1,1],7:[0,1],8:[1,+1]}
    key_point = torch.zeros(1,18)
    scores = torch.zeros(1,9)
    for i in range(9):
        kp_ver_int = kp_int[i]
        if(kp_ver_int[0]<w-1 and kp_ver_int[0]>0):
            kp_ver_int[0] = kp_ver_int[0]
        elif(kp_ver_int[0]>=w-1):
            kp_ver_int[0] = w-2
        elif(kp_ver_int[0]<=0):
            kp_ver_int[0] = 1
        if(kp_ver_int[1]<h-1 and kp_ver_int[1]>0):
            kp_ver_int[1] = kp_ver_int[1]
        elif(kp_ver_int[1]>=h-1):
            kp_ver_int[1] = h-2
        elif(kp_ver_int[1]<=0):
            kp_ver_int[1] = 1
        ...
        radius = 3
        Candidate_point_pb = [
                           kp[:,i,kp_ver_int[1]-1,kp_ver_int[0]-1],
                           kp[:,i,kp_ver_int[1],kp_ver_int[0]-1],                        
                           kp[:,i,kp_ver_int[1]+1,kp_ver_int[0]-1],
                           kp[:,i,kp_ver_int[1]-1,kp_ver_int[0]],
                           kp[:,i,kp_ver_int[1],kp_ver_int[0]],
                           kp[:,i,kp_ver_int[1]+1,kp_ver_int[0]],
                           kp[:,i,kp_ver_int[1]-1,kp_ver_int[0]+1],
                           kp[:,i,kp_ver_int[1],kp_ver_int[0]+1],
                           kp[:,i,kp_ver_int[1]+1,kp_ver_int[0]+1]
                           ]
        max_ind = Candidate_point_pb.index(max(Candidate_point_pb))
        pb = Candidate_point_pb[max_ind]
        coordinate = kp_ver_int + torch.tensor(map_id[max_ind]).cuda().int()
        key_point[:,2*i:2*(i+1)] = coordinate
        scores[:,i:i+1] = pb
    return key_point,scores
    ##search radis = 5 in heatmap

    
def get_kp(ver,inds,kp,opt):
    point_group = torch.zeros(inds.shape[0],inds.shape[1],18)
    scores_group = torch.zeros(inds.shape[0],inds.shape[1],9)
    for i in range(inds.shape[1]):
        group_kp ,kp_scores= find_closed2maiVc(kp,inds[0][i],ver[:,i,:],opt)
     
        point_group[:,i,:] = group_kp
        scores_group[:,i,:] = kp_scores    
    return point_group,scores_group
def ddd_decode_z_anchor(heat, rot, depth, dim, wh=None, reg=None, K=40):
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
    ...
    #import pdb;pdb.set_trace()
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    ys_for_z = ys.view(batch, K, 1)
    anchor_z = torch.ones_like(ys_for_z)
    for i in range(K):
        if(ys_for_z[:,i,0] <=0):
            ys_for_z[:,i,0] =1
        anchor_z[:,i,0] = matchdict[str(int(ys_for_z[:,i,0].detach().cpu().numpy()))][0]
    ...
    rot = rot.view(batch, K, 8)
    depth = _transpose_and_gather_feat(depth, inds)
    depth = depth.view(batch, K, 1) + anchor_z
    dim = _transpose_and_gather_feat(dim, inds)
    dim = dim.view(batch, K, 3)
    clses  = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
...
def ddd_decode_xyz(heat, rot, location, dim, wh=None, reg=None, K=40):
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
    ...
    rot = _transpose_and_gather_feat(rot, inds)
    rot = rot.view(batch, K, 8)
    location = _transpose_and_gather_feat(location, inds)
    location = location.view(batch, K, 3)
    dim = _transpose_and_gather_feat(dim, inds)
    dim = dim.view(batch, K, 3)
    clses  = clses.view(batch, K, 1).float()
    scores = scores.view(batch, K, 1)
    ...
    if wh is not None:
        ...
        wh = wh.view(batch, K, 2)
        detections = torch.cat(
            [xs, ys, scores, rot, location, dim, wh, clses], dim=2)
    else:
        detections = torch.cat(
            [xs, ys, scores, rot, location, dim, clses], dim=2)
